...
from adaptive_phasegate_kbs.utils.tools import StandardScaler
from adaptive_phasegate_kbs.utils.timefeatures import time_features
import warnings
import h5py
...
DEFAULT_DATASET_ROOT = str(FRAMEWORK_ROOT / "dataset")
DEFAULT_EMBED_ROOT = str(FRAMEWORK_ROOT / "Embeddings")


def as_float32_array(array):
    return np.asarray(array, dtype=np.float32)

class Dataset_ETT_hour(Dataset):
    # 默认使用项目内 dataset 目录，避免硬编码绝对路径
    def __init__(self, root_path=DEFAULT_DATASET_ROOT, flag='train', size=None, 
        ...
        self.embed_path = os.path.join(DEFAULT_EMBED_ROOT, data_path_file, embed_version, flag)
    ...
    def __read_data__(self):
        ...
        self.data_x = as_float32_array(data[border1:border2])
        # print(f"Length of self.data_x: {len(self.data_x)}")
        self.data_y = as_float32_array(data[border1:border2])
        self.data_stamp = as_float32_array(data_stamp)
