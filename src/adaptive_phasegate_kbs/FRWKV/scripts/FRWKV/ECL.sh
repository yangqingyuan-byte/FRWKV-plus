...
# Basic parameters (based on original ECL script)
seq_len=96                    # Input sequence length
train_ratio=1.0              # Training data ratio
d_model=512                  # Model dimension
embed_size=16                # Embedding dimension
cuda_id=0                    # GPU ID
learning_rate=5e-4           # Learning rate (using ECL original setting)
dropout=0.0                  # Dropout rate (using ECL original setting)
...
# Random seed
seed=2025
# Training parameters (based on original ECL script)
batch_size=32                # Batch size
train_epochs=30              # Training epochs (can be adjusted based on prediction length)
patience=10                  # Early stopping patience (using ECL original setting)

# ECL dataset specific parameters
enc_in=321                   # ECL dataset input dimension
dec_in=321                   # ECL dataset decoder input dimension
c_out=321                    # ECL dataset output dimension
e_layers=3                   # Encoder layers (using ECL original setting)
...
echo "=========================================="
echo "FRWKV ECL Dataset Multi-prediction Length Experiment"
echo "=========================================="
echo "Model: ${model_name}"
echo "Dataset: ECL (Electricity)"
...
for pred_len in "${pred_lengths[@]}"; do
    ...
    echo "=========================================="
        ...
        python -u run.py \
          ...
          --model_id ECL_frwkv_${seq_len}_${pred_len}_seed${seed} \
          --model ${model_name} \
          --data custom \
          --features M \
          --seq_len ${seq_len} \
          ...
          --plot_mat_flag 0 \
          --num_workers 0 \
          2>&1 | tee -a logs/frwkv/FRWKV_ECL_${seq_len}_${pred_len}_seed${seed}.log
...
echo "Log file list:"
for pred_len in "${pred_lengths[@]}"; do
    log_file="logs/frwkv/FRWKV_ECL_${seq_len}_${pred_len}_seed${seed}.log"
    if [ -f "$log_file" ]; then
        echo "✅ ${log_file}"
    else
...
log_file="logs/frwkv/FRWKV_ECL_${seq_len}_*_seed${seed}.log"
...
echo "- Model: ${model_name}"
echo "- Dataset path: /dataset/electricity/"
echo "- Data file: electricity.csv"
echo "- Input dimension: ${enc_in}"
echo "- Encoder layers: ${e_layers}"
echo "- Learning rate: ${learning_rate}"
