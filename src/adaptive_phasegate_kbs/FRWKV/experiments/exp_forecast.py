        ...
        if self.resume_training and self.resume_epoch > 0:
            full_folder, new_setting = find_most_recently_modified_subfolder(self.args.checkpoints,
                                                                             ...
                                                                             contain_str=[self.args.model_id_ori,
                                                                                          self.args.model])
            if full_folder is None or new_setting is None:
                raise ValueError('Checkpoint saving is disabled by project policy. Please rerun the experiment instead of resuming from disk.')
    ...
    def test(self, setting=None, test=0, test_batch_size=None):
        ...
        if test < 1:
            ...
            if os.path.isfile(checkpoint_path):
                ...
                print('No checkpoint file on disk; using the current in-memory model state.')
        else:
            full_folder, new_setting = find_most_recently_modified_subfolder(self.args.checkpoints,
                                                                             file_name='checkpoint.pth',
                                                                             contain_str=self.args.model_id_ori)
            if full_folder is not None and compare_prefix_before_third_underscore(setting, new_setting):
                ...
                raise ValueError('No checkpoint file found on disk. This project now keeps no saved weights; rerun training to reproduce the model state.')
