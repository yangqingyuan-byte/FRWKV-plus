...
class EncoderLayer(nn.Module):
    # 单个编码层的结构可以概括为：
    # 1) attention(q, k, v)
    # 2) attention 输出做残差相加 + dropout
    # 3) LayerNorm
    # 4) 可选前馈网络 MLP（这里用两个 kernel_size=1 的 Conv1d 实现）
    # 5) MLP 输出再做一次残差相加 + LayerNorm
    #
    # 常规输入形状为 [B, token_num, d_model]。
    # 如果输入是 (q, k) 或 (q, k, v)，则进入非对称/交叉注意力模式，
    # 但最终输出形状仍保持为 [B, token_num, d_model]。
    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None, token_weight=None):
        # 支持两种调用方式：
        # 1) x 是单个张量 -> 自注意力，q = k = v = x
        # 2) x 是 (q, k) 或 (q, k, v) -> 非对称/交叉注意力
        ...
        if self.MLP_flag:
            # 第二子层：逐 token 的前馈网络。
            # 先把 [B, token_num, d_model] 转成 [B, d_model, token_num]，
            # 再用 1x1 Conv1d 在每个 token 位置上做线性映射。
...
class EncoderStack(nn.Module):
    # EncoderStack 是一个“编码层堆叠容器”，整体结构为：
    # input [B, token_num, d_model]
    # -> EncoderLayer x L
    # -> optional final norm
    # -> return x 或 (x, attns)
    #
    # 其中每个 EncoderLayer 都保持外部形状不变，只是在 token 维上建立交互。
    #
    # 在 FRWKV 的 Fre_Trans 路径里，这里的 token_num 实际对应的是通道数 N，
    # 而不是时间步 T。也就是说，频域分支进入 EncoderStack 后，建模重点是
    # “通道与通道之间”的关系。
    #
    # 参数说明：
    # - attn_layers: 若干个 EncoderLayer 组成的堆叠
    # - norm_layer: 编码器末尾可选的归一化层
    # - one_output=True: 只返回最终特征 x
    # - one_output=False: 返回 (x, attns)
    # - CKA_flag: 推理阶段可选地比较首层和末层特征相似度
    ...
    def forward(self, x, attn_mask=None, tau=None, delta=None):
        # x 的常规形状为 [B, token_num, d_model]。
        # 在 Fre_Trans 中，这里通常就是 [B, N, M]：
        # - B: batch size
        # - N: 通道数/变量数，也就是编码器看到的 token 数
        # - M: d_model
        attns = []
        X0 = None  # to make Pycharm happy
        layer_len = len(self.attn_layers)
        for i, attn_layer in enumerate(self.attn_layers):
            # 每一层都保持 [B, token_num, d_model] 的外部形状不变。
        ...
        if isinstance(x, tuple) or isinstance(x, List):
            # 某些非对称注意力场景下，EncoderLayer 会携带附加分支返回，
            # 这里对外只保留主输出特征。
            x = x[0]
