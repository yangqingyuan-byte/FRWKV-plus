...
FRAMEWORK = ROOT
OUT_PATH = ROOT / "jobs" / "experiment_runner_kbs_remaining_tuning_jobs_gpu47.json"
GPU_POOL = [4, 5, 6, 7]
...
def main():
    ...
    # Focused adaptive tuning for the weakest settings surfaced by KBS supplement:
    # - ETTh2-96: adaptive trails CrossBranchPhaseGate
    # - ILI-36: adaptive trails the plain FRWKV backbone
    ...
    for pp, routers, alpha, bias in itertools.product(
        [12, 24, 48],
        [1, 2],
        [0.0, 0.005, 0.01, 0.02],
        [-4.0, -3.5, -3.0],
    ):
        tag = f"kbs_tune_etth2_pl96_pp{pp}_r{routers}_a{alpha}_b{bias}"
        cmd = build_cmd(
            phase_period_len=pp,
    ...
    for pp, routers, alpha, bias in itertools.product(
        ...
        tag = f"kbs_tune_ili_pl36_pp{pp}_r{routers}_a{alpha}_b{bias}"
        cmd = build_cmd(
            phase_period_len=pp,
            phase_num_routers=routers,
            phase_alpha_init=alpha,
